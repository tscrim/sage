r"""
Bijection classes for type `E_6^{(2)}`

Part of the (internal) classes which runs the bijection between rigged
configurations and KR tableaux of type `E_6^{(2)}`.
...
class KRTToRCBijectionTypeE6Twisted(KRTToRCBijectionAbstract):
    r"""
...
class RCToKRTBijectionTypeE6Twisted(RCToKRTBijectionAbstract):
    r"""
...
-2*Lambda[0] + Lambda[1]
    ...
    def _endpoint(self, r):
        ...
        EXAMPLES::
            ...
            True
        """
        C = CrystalOfLetters(self.rigged_con.parent().cartan_type().classical())
        if r == 4:
            return C((-1,4))
        if r == 1:
