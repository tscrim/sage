...
class HeckeAlgebraRepresentation(CombinatorialFreeModule):
    ...
    def __init__(self, base_ring, basis_keys, prefix, q1, q2, cartan_type=None, module_action="left"):
        ...
        if q1 not in base_ring or q2 not in base_ring:
    ...
    def _Tr_matrix(self,r):
        r"""
        Return the matrix that describes the action of `T_r`
        on the seminormal representation ``self``.
        """
        ...
        for s in self._indices:
            for (tab,coeff) in self(s).T(r):
                mat[(self._indices.rank(s),self._indices.rank(tab))] = coeff
        M = matrix(self._base, mat)
        M.set_immutable()
        return M

    @cached_method
    def T_matrix(self, *args):
        r"""
        Return the matrix that describes the action of `T_w`
        on the seminormal representation `self`.
        ...
        EXAMPLES::
            ...
            [   r3*I/(1 + q^2)      1/(-q - q^3)]
        """
        M = prod(self._Tr_matrix(r) for r in args)
        M.set_immutable()
        return M
    ...
    def action(self, a):
        ...
        return a.parent().linear_combination((a.parent().T(*w.reduced_word()), c)
                                             for (w,c) in a.to_T_basis())

    ## check that the algebra's relations are satisfied on the representation ---------------------------

    def _test_relations(self, verbose=False, **options):
        r"""
        Return ``True`` if all of the relations are satisfied on ``self``
        and ``False`` otherwise.
        ...
        for t in self._indices:
            ...
            for r in range(1, self.size()):
                ...
                tester.assertEquals(self(t).T(r,r),
                                    self._q_sum * self(t).T(r) + self._q_prod * self(t),
                    ('Quadratic relation failed for T_{r} on {term}'.format(r,self(t)))
                )
            if r < self.size()-1:
                # check length 3 braid relation on self(t): T_rT_{r+1}T_r = T_{r+1}T_rT_{r+1}
                tester.assertEquals(self(t).T(r, r+1, r),
                                    self(t).T(r+1, r, r+1),
                    ('Length 3 braid relation failed for T_{r} on ({term})'.format(r,self(t)))
                )
            for s in range(r+2, self.size()):
                # check length 2 braid relation on self(t): T_rT_s=T_sT_r if |r-s|>1
                tester.assertEquals(self(t).T(r,s),
                                    self(t).T(s,r),
                    ('Length 2 braid relation failed for T_{r} on ({term})'.format(r,self(t)))
                )
        return True
    ...
    class Element(CombinatorialFreeModule.Element):
        def _Tr(self, r):
            r"""
            This is a helper function for computing the action of `T_r`
            upon the element ``self`` of the seminormal representation.
            ...
            """
            return self.parent().linear_combination((self.parent()._T_on_basis(b,r), coeff)
                                                    for (b,coeff) in self)
        ...
        def _T_right_action(self,*args):
            r"""
            Compute the action of `T_w` on self, where `w` is the
            permutation corresponding to the (reduced) word ``*args``.
...
class SeminormalRepresentation_generic(HeckeAlgebraRepresentation):
    r"""
    This is a private class that implements the broad skeleton of the
    seminormal representations of the (cyclotomic) Hecke algebras of type `A`.
    The derived classes of this class need to:

    - define the ``base_ring`` for the representation and set ``q1`` and ``q2``
    ...
    - implement the four methods:
      ...
      - :meth:`_rho_and_beta_degenerate`
      - :meth:`_rho_and_beta_degenerate_level_one`
      - :meth:`_rho_and_beta_nondegenerate`
      - :meth:`_rho_and_beta_nondegenerate_level_one`
    """
    @staticmethod
    def __classcall__(cls, shape, q1=None, q2=-1, charge=None,
                      degenerate=False, prefix='?', **kwargs):
        """
        ...
        EXAMPLES::
            ...
            sage: SeminormalRepresentation([3,2,1])
            SeminormalRepresentation([3,2,1])
        """
        try:
    ...
    def __init__(self, shape, q1, q2, prefix, degenerate, base_ring, **kwargs):
        ...
        indexed by the partition ``shape`` and with the basis elements
        labelled by ``prefix``.
    ...
    def _test_relations(self, verbose=False, **options):
        r"""
        Return ``True`` if the action of the algebra of the representation
        ``self`` repects all of the algebras relations and ``False`` otherwise.
        """
        tester = self._tester(**options)
        ...
        for t in self._indices:
            ...
            for r in range(1, self.size()+1):
                # check quadratic relations on self(t)
                tester._assertEquals(self(t).L(r),
                                     self._tableau_content(t,r)*self(t),
                    ('L_{r} is not acting with the expected eigenvalue on {term}'.format(r,self(t)))
                )
        return super(SeminormalRepresentation_generic, self)._test_relations(verbose, **options)

    def shape(self):
        r"""
        Return the partition that indexes the seminormal representation
        ``self``.
    ...
    def size(self):
        r"""
        Return the size of the partition that indexes the seminormal
        representation ``self``.
    ...
    def generator(self):
        r"""
        Return a generator, as a module for the Hecke algebra, of
        the Specht module ``self``.
    ...
    def _repr_(self):
        r"""
        Return a string representation of the seminormal representation
        ``self``.
    ...
    def _repr_term(self,tab):
        ...
        EXAMPLES::
            ...
            2*v(1,2/3) + 2*v(1,3/2)
        """
        return '{}({})'.format(self._prefix, tab._repr_compact() )

    def _element_constructor_(self,t):
        r"""
        We override the default element constructor in order to coerce
        lists into standard tableaux.
        ...
        EXAMPLES::
            ...
            f(1,2,3/4,5)
        """
        return self.monomial(self._indices(t))

    def _tableau_content_degenerate(self, t, r):
        r"""
        Return the content of the integer ``r`` in the tableau ``t``
    ...
    def _tableau_content_nondegenerate_level_one(self, t, r):
        r"""
        Return the content of the integer ``r`` in the tableau ``t``
        in the nondegenerate level 1 case.
        """
    ...
    def _T_on_basis(self,tab,r):
        r"""
        Compute the action of `T_r` on the basis element `f_tab`,
        where the action of `T_r` is as follows:

        .. MATH::

           f_t T_r = \frac{-q^{-1}}{[d]}f_t + \alpha_d f_s

        where `d = c_r(t) - c_{r+1}(t)` is the axial distance from `r`
        to `r+1` in `t` and
        ...
        .. MATH::
         ...
         The whole point of choosing this particular basis is that if the
         module is indexed by a self-conjugate partition then it splits
         when it is restricted to the alternating Hecke algebra.
    ...
    class Element(HeckeAlgebraRepresentation.Element):
        def L(self, *arg):
            r"""
            Return the action of `L(r_1, r_2, \ldots)` on the module
            element ``self``.
            """
...
class SeminormalRepresentation(SeminormalRepresentation_generic):
    def __init__(self, shape, q1, q2, charge, degenerate, **kwargs):
        ...
        if q1 is None:
            if q2 is None:
                ...
                F = FractionField(PolynomialRing(ZZ, 'u,v'))
                q1,q2 = F.gens()
            else:
                # the default parameters for the relation (T_r-q)(T_r+1)=0
                F = FractionField(PolynomialRing(ZZ, 'q'))
                q1 = F.gen()
        else:
    ...
    def _rho_and_beta_degenerate_level_one(self, c, d):
        return (ZZ(-1) / self.__quantum_integer(c-d),
                self.__quantum_integer(c-d+1) / self.__quantum_integer(c-d))

    def _rho_and_beta_nondegenerate_level_one(self, c, d):
        return ((self._q1-self._q2) * self._q1**c / (self._q1**c-self._q2**d),
                (self._q1**(c+1)-self._q2**d) / (self._q1**c-self._q2**d))
...
class SeminormalRepresentation_Orthogonal(SeminormalRepresentation_generic):
    def __init__(self, shape, prefix='y'):
        ...
        self._shape = shape
        self._prefix = prefix
        Fq = FractionField(PolynomialRing(ZZ, 'q'))
        self.q = Fq.gen()
    ...
    def _rho_and_beta_degenerate_level_one(self, c, d):
        return (ZZ(-1) / self.__quantum_integer(c-d),
                self.__quantum_integer_root(c-d+1) / self.__quantum_integer(c-d))
...
class SeminormalRepresentation_Murphy(SeminormalRepresentation_generic):
    ...
    def _rho_and_beta_degenerate_level_one(self, c, d):
        if c > d:
            return ZZ(-1) / self.__quantum_integer(c-d), ZZ.one()
        else:
            beta = (self.__quantum_integer_root(c-d+1)
                    * self.__quantum_integer_root(c-d+1)
                    / self.__quantum_integer(c-d)**2)
            return ZZ(-1) / self.__quantum_integer(c-d), beta
    ...
    def _murphy(self,tab):
        r"""
        Return the Murphy basis element indexed by the tableau ``tab``
        as a linear combination of seminormal basis elements.
        """
...
class SeminormalRepresentation_Alternating(SeminormalRepresentation_generic):
    ...
    def _quantum_integer_root(self,d):
        ...
        """
        if d >= 0:
            return self.__quantum_integer_root[d]
        return -self.q**(2*d) * self.__quantum_integer_root[-d]

    def _rho_and_beta_degenerate_level_one(self, c, d):
        rho = ~(self.q**(2*d+1) * self._quantum_integer(-d))
        if d > 0:
            beta = (self.I * self._quantum_integer_root(d+1)
                    * self._quantum_integer_root(d-1)
                    / self._quantum_integer(d))
        else:
            beta = (-self.I * self._quantum_integer_root(-d+1)
                    * self._quantum_integer_root(-d-1)
                    / self._quantum_integer(-d))
